                                        ...
                                        <div class="col-lg-4 col-sm-8">
                                            <div class="input-group">
                                                ...
                                                <span class="input-group-btn">
                                                    <button class="btn btn-default btn-flat" type="button" onclick="jump.layerModal('/admin/attachments/select?id=userAvatar','<@spring.message code="common.js.all-attachment" />')"><@spring.message code='common.btn.choose' /></button>
                                                </span>
                                            </div>
                                        </div>
                                    </div>
...
<script type="application/javascript" id="footer_script">
    ...
    function saveUser(option) {
        ...
        $.post('/admin/profile/save',param,function (data) {
            if(data.code === 1){
                jump.showMsgAndRedirect(data.msg,'success',1000,'/admin/profile',"${options.admin_pjax!'true'}");
            }else{
    ...
    function changPass() {
        ...
        if(newPass!==reNewPass){
            jump.showMsg("<@spring.message code='admin.user.profile.form.password.no-same' />",'error',2000);
            return;
        }
